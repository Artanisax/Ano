...
from seanet.conv import NormConv2d
...
class DiscriminatorP(torch.nn.Module):
    ...
    def forward(self, x, return_fmap=True):
        fmap = []
...
class MultiPeriodDiscriminator(torch.nn.Module):
    ...
    def forward(self, y, y_hat, return_fmaps=True):
        y_d_rs = []
        y_d_gs = []
        fmap_rs = []
        fmap_gs = []
...
class MultiScaleSTFTDiscriminator(nn.Module):
    ...
    def forward(self, y: torch.Tensor, y_hat: torch.Tensor, return_fmaps: bool = True) -> DiscriminatorOutput:
        logits = []
        logits_fake = []
        fmaps = []
        fmaps_fake = []
        for disc in self.discriminators:
            logit, fmap = disc(y, return_fmap=return_fmaps)
            logits.append(logit)
            logit_fake, fmap_fake = disc(y_hat, return_fmap=return_fmaps)
